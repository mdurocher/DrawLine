...
public class LensBlurFilter extends AbstractBufferedImageOp {
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    // Create the kernel
    final double polyAngle = Math.PI / sides;
    final double polyScale = 1.0f / Math.cos(polyAngle);
    final double r2 = radius * radius;
    final double rangle = Math.toRadians(angle);
    float total = 0;
    int i = 0;
    for (int y = 0; y < h; y++)
      for (int x = 0; x < w; x++) {
        final double dx = x - w / 2f;
        final double dy = y - h / 2f;
    ...
    for (int tileY = -iradius; tileY < height; tileY += tileHeight - 2 * iradius)
      for (int tileX = -iradius; tileX < width; tileX += tileWidth - 2 * iradius) {
        ...
        // Create a float array from the pixels. Any pixels off the edge of the
        // source image get duplicated from the edge.
        i = 0;
        for (int y = 0; y < h; y++) {
          final int imageY = y + tileY;
          int j;
          if (imageY < 0)
            j = fy;
        ...
        for (int y = 0; y < w; y++) {
          final int ym = y ^ row_flip;
          final int yi = ym * cols;
          for (int x = 0; x < w; x++) {
            final int xm = yi + (x ^ col_flip);
            final int a = (int) ar[0][xm];
            int r = (int) ar[1][xm];
            int g = (int) gb[0][xm];
            int b = (int) gb[1][xm];
