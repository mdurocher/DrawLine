...
public class FadeFilter extends PointFilter {
  ...
  public float symmetry(float x, float b) {
    /*
     * int d = (int)(x / b); x = ImageMath.mod(x, b); if ((d & 1) == 1) return
     * b-x; return x;
     */
   ...
   * float sideAngle = 2*Math.PI / sides; float angle = Math.atan2(y, x); float
   * r = Math.sqrt(x*x + y*y); float t = ImageMath.mod(angle, sideAngle) /
   * sideAngle; if (t > 0.5) t = 1.0-t; }
   */
