...
public class DisplaceFilter extends TransformFilter {
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    final BufferedImage dm = displacementMap != null ? displacementMap : src;
    ...
    i = 0;
    for (int y = 0; y < dh; y++) {
      final int j1 = (y + dh - 1) % dh * dw;
      final int j2 = y * dw;
      final int j3 = (y + 1) % dh * dw;
      for (int x = 0; x < dw; x++) {
        final int k1 = (x + dw - 1) % dw;
        final int k2 = x;
        final int k3 = (x + 1) % dw;
