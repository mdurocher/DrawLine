...
public class FieldWarpFilter extends TransformFilter {
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    if (inLines != null && outLines != null) {
      ...
      for (int line = 0; line < inLines.length; line++) {
        final Line l = intermediateLines[line] = new Line(ImageMath.lerp(amount, inLines[line].x1, outLines[line].x1),
            ImageMath.lerp(amount, inLines[line].y1, outLines[line].y1), ImageMath.lerp(amount, inLines[line].x2,
                outLines[line].x2), ImageMath.lerp(amount, inLines[line].y2, outLines[line].y2));
        l.setup();
        inLines[line].setup();
      }
  ...
  protected void transformInverse(int x, int y, float[] out) {
    ...
    for (int line = 0; line < inLines.length; line++) {
      ...
      sumY += (v - y) * weight;
      // if (x % 10 == 0&&y ==
      // 20)System.out.println("distance="+distance+" weight="+weight+" sumX="+sumX+" sumY="+sumY+" u="+u+" v="+v);
      totalWeight += weight;
    }
