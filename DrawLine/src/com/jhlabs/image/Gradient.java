...
public class Gradient extends ArrayColormap implements Cloneable {
  ...
  public void addKnot(int x, int color, int type) {
    final int[] nx = new int[numKnots + 1];
    final int[] ny = new int[numKnots + 1];
    final byte[] nt = new byte[numKnots + 1];
  ...
  public void randomize() {
    ...
    for (int i = 0; i < numKnots; i++) {
      ...
      yKnots[i] = 0xff000000 | (int) (255 * Math.random()) << 16 | (int) (255 * Math.random()) << 8
          | (int) (255 * Math.random());
      knotTypes[i] = RGB | SPLINE;
    }
    xKnots[0] = -1;
  ...
  private void rebuildGradient() {
    ...
    for (int i = 1; i < numKnots - 1; i++) {
      final float spanLength = xKnots[i + 1] - xKnots[i];
      int end = xKnots[i + 1];
      if (i == numKnots - 2)
        end++;
      for (int j = xKnots[i]; j < end; j++) {
        final int rgb1 = yKnots[i];
        final int rgb2 = yKnots[i + 1];
        ...
        final int type = getKnotType(i);
        final int blend = getKnotBlend(i);
        ...
        if (j >= 0 && j <= 255) {
          ...
          case HUE_CCW:
            if (type == HUE_CW) {
              ...
              hsb1[0] += 1.0f;
            final float h = ImageMath.lerp(t, hsb1[0], hsb2[0]) % ImageMath.TWO_PI;
  ...
  public void setColor(int n, int color) {
    final int firstColor = map[0];
    final int lastColor = map[256 - 1];
    if (n > 0)
      for (int i = 0; i < n; i++)
  ...
  /**
   * Set the values of all the knots. This version does not require the "extra"
   * knots at -1 and 256
   *
   * @param x
   *          the knot positions
  ...
  public void splitSpan(int n) {
    final int x = (xKnots[n] + xKnots[n + 1]) / 2;
