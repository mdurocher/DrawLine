...
public class TransitionFilter extends AbstractBufferedImageOp {
   ...
   *          the filter to use
   * @param property
   *          the filter property which is changed over the transition
   * @param minValue
   *          the start value for the filter property
   * @param maxValue
   *          the end value for the filter property
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    final float itransition = 1 - transition;

    final Graphics2D g = dst.createGraphics();
    if (transition != 1) {
      final float t = minValue + transition * (maxValue - minValue);
      prepareFilter(t);
      g.drawImage(src, filter, 0, 0);
    }
    if (transition != 0) {
      g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, transition));
      final float t = minValue + itransition * (maxValue - minValue);
      prepareFilter(t);
   ...
   * implementation sets the given filter property, but you could override this
   * method to make other changes.
   *
   * @param transition
  ...
  /*
   * public void setFilter( BufferedImageOp filter ) { this.filter = filter; }
   * 
