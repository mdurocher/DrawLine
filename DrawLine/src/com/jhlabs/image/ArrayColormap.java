...
public class ArrayColormap implements Colormap, Cloneable {
  ...
  public int getColor(float v) {
    /*
     * v *= 255; int n = (int)v; float f = v-n; if (n < 0) return map[0]; else
     * if (n >= 255) return map[255]; return ImageMath.mixColors(f, map[n],
     * map[n+1]);
     */
    int n = (int) (v * 255);
    if (n < 0)
  ...
  public void setColorInterpolated(int index, int firstIndex, int lastIndex, int color) {
    final int firstColor = map[firstIndex];
    final int lastColor = map[lastIndex];
